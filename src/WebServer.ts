...
if (!ethereumUrl)
  throw new Error('ETHEREUM_URL environment variable must be defined.');
const pollingFrequencySecondsString = process.env.POLLING_FREQUENCY_SECONDS;
if (!pollingFrequencySecondsString)
  throw new Error(
    `POLLING_FREQUENCY_SECONDS environment variable must be defined.`
  );
const pollingFrequencySeconds = Number.parseFloat(
  pollingFrequencySecondsString
);
if (
  ...
  throw new Error(
    `POLLING_FREQUENCY_SECONDS must be an integer number of seconds between 1 and 3600`
  );
...
new Koa()
  ...
  .use(async (context) => {
    ...
    for (let i = 1; i <= 4; ++i) {
      context.body[`percentile_${i}`] = `${
        (await gasOracle.getPercentile(i)) / 10 ** 9
      } nanoeth`;
    }
  ...
  .listen(80);
console.log('Gas service starting... ');
