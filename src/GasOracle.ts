...
export class GasOracle {
  ...
  private readonly blockMinGasPrices = new Deque<{
    hash: string;
    minGas: number;
  }>();
  ...
  public constructor(ethereumUrl: string, pollingFrequency: number) {
    ...
    this.reconcileAgedBlock(50)
      ...
      .then(() =>
        setInterval(() => this.reconcileLatestBlock(), pollingFrequency * 1000)
      );
  }
  ...
  public getPercentile = async (percentile: number): Promise<number> => {
    ...
    const sorted = this.blockMinGasPrices
      .map((x) => x.minGas)
      .sort((a, b) => a - b);
    if (sorted.length <= 0)
      throw new Error(
        `Please wait until the service has fetched at least one block.`
      );
  ...
  public getLatestBlockNumber = () => {
    ...
    return maybeLatestBlock !== null
      ? parseInt(maybeLatestBlock.number, 16)
      : 0;
  };
  ...
  private onBlockRemoved = (block: Block) => {
    ...
    if (poppedBlock.hash !== block.hash)
      throw new Error(
        `Received notification to remove block ${block.hash} but found ${poppedBlock.hash} on the top of the stack.`
      );
  };
  ...
  private getAgedBlock = async (numBlocksBack: number) => {
    ...
    const targetBlock = Math.max(
      0,
      parseInt(latestBlock.number, 16) - numBlocksBack
    );
  ...
  private getBlockByNumber = async (blockNumber: number | 'latest') => {
    ...
    const response = await fetch(this.ethereumUrl, {
      method: 'POST',
    ...
    if (result.error)
      throw new Error(
        `Failed to fetch block (${blockNumber}): ${result.error.code}\n${result.error.message}`
      );
    if (!result.result)
      throw new Error(
  ...
  private getBlockByHash = async (hash: string) => {
    ...
    if (result.error)
      throw new Error(
        `Failed to fetch block with hash ${hash}: ${result.error.code}\n${result.error.message}`
      );
    if (!result.result)
      throw new Error(
        `Received unexpected response:\n${JSON.stringify(
